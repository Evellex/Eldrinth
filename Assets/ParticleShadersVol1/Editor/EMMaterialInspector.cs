...
namespace UnityEditor
{
	internal class EMMaterialInspector : ShaderGUI
	{
		...
		private static class Styles
		{
			...
			public static GUIContent m_ForwardPathWarning = new GUIContent ("! Soft Particles are not recomended to use in Forward Rendering Path due to massive performance impact !");
		}
			...
			if (m_InvFade != null) {
				...
				EditorGUI.EndChangeCheck();
				{
					...
					if (softParticles)
					{
						...
						if (PlayerSettings.renderingPath == RenderingPath.Forward)
							EditorGUILayout.HelpBox(Styles.m_ForwardPathWarning.text, MessageType.Warning);
					}
